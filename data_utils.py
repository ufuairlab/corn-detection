...
class SegmentationDataset(Dataset):
    ...
    def __init__(self, json_folder, img_folder, is_train, class_to_id, resolution = (1486, 992), augmentation = False, transform=None):
        """
        Args:
    ...
    def __getitem__(self, idx):
        ...
        #print("img_number_str: " + str(img_number_str))

	# Abre Json
        gt_json = json.load(open(gt_file, 'r'))
        print("name 1" + str(osp.join(img_number_str + '.jpg')))
        print("name 2" + str(osp.join(self.img_folder, img_number_str + '.jpg')))
	# Abre imagem
        img_np = cv2.imread(osp.join(self.img_folder, img_number_str + '.jpg'), cv2.IMREAD_IGNORE_ORIENTATION + cv2.IMREAD_COLOR)
        #img_np = cv2.imread(osp.join(img_number_str + '.jpg'), cv2.IMREAD_IGNORE_ORIENTATION + cv2.IMREAD_COLOR)
        #print("img shape: " + str(img_np.shape))
	...
	# Para todos poligonos
        ...
        if self.transform:
            ...
            img_pt = self.transform(img_pt)
            label_pt = self.transform(label_pt)
